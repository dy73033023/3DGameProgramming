...
CGameFramework::CGameFramework()
{
	...
	m_nWndClientWidth = winWidth;
	m_nWndClientHeight = winHeight;
...
void CGameFramework::CreateSwapChain()
{
	...
	m_pdxgiFactory->CreateSwapChainForHwnd(m_pd3dCommandQueue, m_hWnd,
		&dxgiSwapChainDesc, NULL, NULL, (IDXGISwapChain1
			**)&m_pdxgiSwapChain);
	//스왑체인을 생성한다. 
	...
	wParam, LPARAM lParam)
{
	...
	case WM_KEYUP:
		...
		case VK_RETURN:
			break;
			//“F9” 키가 눌려지면 윈도우 모드와 전체화면 모드의 전환을 처리한다. case VK_F9:
...
void CGameFramework::ProcessInput()
{
	...
	if ((dwDirection != 0) || (cxDelta != 0.0f) || (cyDelta != 0.0f))
	{
		...
		else
		{
			...
			if (dwDirection) m_pPlayer->Move(dwDirection, 900 * m_GameTimer.GetTimeElapsed(),
				true);
		}
	}
